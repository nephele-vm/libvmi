...
static addr_t
unikraft_get_taskstruct_addr_from_pid(
    vmi_instance_t vmi,
    vmi_pid_t pid)
{
    addr_t next_process = 0;
    vmi_pid_t task_pid = -1;
    unikraft_instance_t unikraft_instance = NULL;
    int pid_offset = 0;
    ...
    unikraft_instance = vmi->os_data;

    pid_offset = unikraft_instance->pid_offset;
    next_process = vmi->init_task;

    do {
        vmi_read_32_va(vmi, next_process + pid_offset, 0,
                (uint32_t*) &task_pid);
...
static addr_t
unikraft_get_taskstruct_addr_from_pgd(
    vmi_instance_t vmi,
    addr_t pgd)
{
    addr_t next_process = 0;
    ...
    addr_t *pgd)
{
    ...
    vmspace_offset = unikraft_instance->vmspace_offset;
    pgd_offset = unikraft_instance->pgd_offset;
    pmap_offset = unikraft_instance->pmap_offset;

    proc_addr = unikraft_get_taskstruct_addr_from_pid(vmi, pid);
    if (!proc_addr) {
    ...
    vmi_pid_t *pid)
{
    ...
    proc_addr = unikraft_get_taskstruct_addr_from_pgd(vmi, pgd);
    if (!proc_addr) {
