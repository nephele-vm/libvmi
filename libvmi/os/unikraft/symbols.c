...
static status_t
unikraft_system_map_symbol_to_address(
    vmi_instance_t vmi,
    const char *symbol,
    addr_t *address)
{
    ...
    if ((NULL == unikraft_instance->sysmap) || (strlen(unikraft_instance->sysmap) == 0)) {
        errprint("VMI_WARNING: No unikraft sysmap configured\n");
        goto done;
    }

    row = safe_malloc(MAX_ROW_LENGTH);
    if ((f = fopen(unikraft_instance->sysmap, "r")) == NULL) {
        fprintf(stderr,
                "ERROR: could not find System.map file after checking:\n");
        fprintf(stderr, "\t%s\n", unikraft_instance->sysmap);
        fprintf(stderr,
    ...
    addr_t* address)
{
    ...
    if (!unikraft_instance->sysmap && !json_profile(vmi)) {
        errprint("VMI_WARNING: No unikraft sysmap and json profile configured\n");
        goto done;
    }

    if (unikraft_instance->sysmap)
        ret = unikraft_system_map_symbol_to_address(vmi, symbol, address);
    else
